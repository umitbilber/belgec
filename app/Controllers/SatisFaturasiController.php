...
class SatisFaturasiController extends BaseController
{
    private SatisFaturasiServiceInterface $service;
private CariServiceInterface $cariService;
private StokServiceInterface $stokService;

    public function __construct(
    SettingsServiceInterface $settingsService,
    SatisFaturasiServiceInterface $service,
    ...
    {
        $this->guardIzin($response, 'satis_fatura.goruntule');
...
$edmOnbilgi = [
    ...
    'kalemler'    => $edmKalemler,
                ...
                $response->view('satis_fatura.index', [
...
'filtreler' => [
    ...
    {
        $this->guardIzin($response, 'satis_fatura.ekle');
        
        $this->guardCsrf($request, $response);

        try {
            $this->service->create($request->input());
            $this->auditLog('ekle', 'satis_fatura', $id);
            $response->redirect(url('satis-faturalari'));
        } catch (Throwable $e) {
            $response->redirect(url('satis-faturalari?hata=' . urlencode($e->getMessage())));
        }
    }

    public function update(Request $request, Response $response): void
    {
        $this->guardIzin($response, 'satis_fatura.duzenle');
        ...
        try {
            ...
            $this->auditLog('duzenle', 'satis_fatura', $id);
            $response->redirect(url('satis-faturalari'));
        } catch (Throwable $e) {
            $response->redirect(url('satis-faturalari?hata=' . urlencode($e->getMessage())));
        }
    ...
    {
        $this->guardIzin($response, 'satis_fatura.sil');

        $id = (int) $request->input('id', 0);

        try {
            $this->service->delete($id);
            $this->auditLog('sil', 'satis_fatura', $id);
            $response->redirect(url('satis-faturalari'));
        } catch (Throwable $e) {
            $response->redirect(url('satis-faturalari?hata=' . urlencode($e->getMessage())));
        }
